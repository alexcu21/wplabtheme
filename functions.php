 ...
 */
function wplabtheme_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on WP Lab Them, use a find and replace
		* to change 'wplabtheme' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'wplabtheme', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'wplabtheme' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'wplabtheme_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'wplabtheme_setup' );
 ...
 */
function wplabtheme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'wplabtheme_content_width', 640 );
}
add_action( 'after_setup_theme', 'wplabtheme_content_width', 0 );
 ...
 */
function wplabtheme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'wplabtheme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'wplabtheme' ),
...
add_action( 'widgets_init', 'wplabtheme_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'wplabtheme_scripts' );
